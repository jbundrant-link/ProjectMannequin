...
**Identification `[CONFIRMED]`:** *Tekken 3* (PlayStation, Namco). The main‑menu **TEKKEN 3** logo (~0:02)
...
**Per‑stage anatomy `[CONFIRMED]`:** *title card ("STAGE n / NAME") → "READY?" → advance right through
soldier waves → boss → STAGE CLEAR tally → NOW LOADING → next.*
...
* **Core loop:** *advance → crowd‑fight soldiers → boss duel → clear → next stage,* under a countdown.
* **Secondary loop:** *manage health & timer via +LIFE/+SEC pickups* while maximizing score.
* **Meta loop:** *beat your score toward the fixed HI‑SCORE;* (in the wider game, completing the mode unlocks
...
**What it is:** *Tekken Force*, a compact side‑scrolling **beat‑'em‑up mode inside Tekken 3** that reuses the
...
*Prepared from frame‑level analysis of the cited video. Items are tagged `[CONFIRMED]` / `[INFERENCE]` /
