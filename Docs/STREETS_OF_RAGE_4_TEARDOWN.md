...
**Game identification `[CONFIRMED]`:** _Streets of Rage 4_. The title logo appears on the attract screen
...
| 3   | **Story prologue**          | CK 4:00; LP 0:05                       | Animated crawl over a night skyline: _"Ten years have passed since the fall of Mr. X and his Syndicate… brilliant Dr. Zan. Together these four vigilantes stand against the Y Syndicate… Streets of Rage."_ `[CONFIRMED]`                                         |
...
> lets you hot‑swap characters. The design fact is: _one character is chosen per run._
...
| **Board vehicles as arenas**                  | LP 110:00 (jet), ~25:00 (ship)  | Stages _are_ set on a train, cargo ship, jet — the vehicle is the arena, not driven | Contextual          |
...
**Per‑stage flow `[CONFIRMED]`:** _Title card → arena waves (escalating count/toughness) → optional mid‑boss
→ traversal → boss with named health bar → clear → results/rank → (cutscene) → map → next title card._
...
- **Core loop:** _advance → screen locks → clear the crowd → GO ▸ → boss → clear._
- **Secondary loop:** _manage HP ↔ specials ↔ stars ↔ weapons_ while maximizing combo/score.
- **Meta loop:** _score → extra lives + per‑stage letter rank_ (mastery/replay driver); character variety +
  co‑op for re‑runs.
...
**What it is:** _Streets of Rage 4_, a polished, linear, stage‑based **2.5D beat‑'em‑up** where one of five
...
**Stage structure & progression:** _title card → escalating waves → boss (named health bar) → results/rank →
comic cutscene → **isometric map** to the next named location → repeat._ Progression is **fixed/linear**;
...
_Prepared from frame‑level analysis of the two cited videos. Items are tagged `[CONFIRMED]` / `[INFERENCE]`
