...
- World Warrior Pavilion Rolling Log — 15/16 World Warrior's first HAZARD zone (not a breakable prop): a vermilion-lacquered rolling log bound in indigo rope wraps with a saffron accent band (motifs sourced from the approved Pavilion Rack Chest and Pavilion Circuit backdrop), LinearSweep behavior reusing the Archive Intake Tram's timing envelope, calibrated 1.00-unit diameter (24.4% of true mannequin height, reasoned against authored stage geometry rather than the character-height bands), wired into Pavilion Circuit's first encounter alongside the training dummy. A real runtime capture confirms its authored warning text renders correctly in-engine. This corrects an earlier placement error where both hazards were built with Dojo Approach motifs and wired into Dojo Approach; `MASTER_IMPLEMENTATION_PLAN.md`'s authoritative per-stage concept list assigns these hazards to Pavilion Circuit, not Dojo Approach ("minimal hazard pressure, breakable training props").
- World Warrior Pavilion Falling Weight — 15/16 World Warrior's second hazard zone: a vermilion-lacquered training weight bound in an indigo rope harness with a saffron accent band and a red medallion strike-target mark, FallingStrike behavior matching the Archive Repository Falling Shelf's family, calibrated 1.20-unit scale (29.2%), wired into Pavilion Circuit's second encounter alongside the rack chest. Both hazards are verified content-distinct from each other and from the existing dummy/rack chest in a four-way silhouette comparison, and both are fully covered by deterministic StageHazardRuntime phase-transition assertions.
...
3. **DONE:** The training dummy, all four World Warrior breakable crates (Sparring Supply Crate, Pavilion Rack Chest, Grand Tournament Champion's Trophy Podium, Champion's Lantern Urn), the Vitality Gourd, Focus Drum, and Judge's Laurel Fan pickups, and World Warrior's first two hazard zones (Pavilion Rolling Log, Pavilion Falling Weight) are all runtime approved. The Rack Chest's initial calibration read far too small next to the player character (measured 44% of true mannequin height) and was corrected to 63%; the Trophy Podium (56%), Lantern Urn (60%), Rolling Log (24%), and Falling Weight (29%) all applied the measured **Proportion and scale calibration** gate from the start and needed no correction. The Sparring Supply Crate (measured 32%) still needs a proportion re-review, scheduled as part of the `Full Proportion Sizing Sweep` in `PROJECT_HANDOFF.md`. Spectator/crowd layers and arena destruction remain pending.
