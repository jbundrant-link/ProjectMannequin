#!/usr/bin/env python3
"""Create gameplay-scale review proxies for the training-dummy pilot."""
...
PILOT = Path(
    ...
    "world_warrior_training_dummy_style_pilot_v1.png"
)
STAGE_CAPTURE = Path(
    "Artifacts/StyleCalibration/"
    "world_warrior_dojo_prodigy_kenzo_idle_runtime_1280x720.png"
)
STAGE_PROXY = Path(
    "Artifacts/StyleCalibration/"
    "world_warrior_training_dummy_pilot_v1_stage_scale_proxy.png"
)
READABILITY_STRIP = Path(
    "Artifacts/StyleCalibration/"
    "world_warrior_training_dummy_pilot_v1_readability_strip.png"
)
...
def main() -> int:
    ...
    stage_figure = resize_to_height(figure, 190)
    stage.alpha_composite(stage_figure, (790, 442 - stage_figure.height))
    stage_draw = ImageDraw.Draw(stage)
    stage_draw.rectangle((786, 248, 1015, 276), fill=(18, 19, 24, 205))
    stage_draw.text(
        (796, 256),
    ...
    panel_width = 240
    label_height = 38
    strip = checkerboard(panel_width * len(heights), 256)
    ...
    for index, height in enumerate(heights):
        ...
        draw.text((panel_x + 12, 11), f"{height}px", fill=(245, 247, 250, 255))
