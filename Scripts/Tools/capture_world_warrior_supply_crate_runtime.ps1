...
$intact = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_supply_crate_intact_runtime_$suffix.png"
$drop = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_supply_crate_drop_runtime_$suffix.png"
$collection = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_supply_crate_collection_runtime_$suffix.png"
$log = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_supply_crate_runtime_$suffix.log"
$environment = [ordered]@{
    ...
    PROJECT_MANNEQUIN_STAGE_INDEX = '1'
    ...
    PROJECT_MANNEQUIN_LADDER_PROP_SPRITE_SUFFIX = 'world_warrior_supply_crate_style_v1.png'
...
try {
    ...
    $meterDelta = if ($collectionLines.Count -eq 1) {
    ...
    try {
        $passed = $exitCode -eq 0 `
            ...
            -and $summary.Value -match 'stage=1' `
        ...
        if (-not $passed) {
            throw "Sparring Supply Crate runtime capture failed validation at $suffix."
        }
    }
    finally {
        if ($intactImage) {
