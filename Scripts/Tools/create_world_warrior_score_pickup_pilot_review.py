#!/usr/bin/env python3
"""Create gameplay-scale review proxies for the Judge's Laurel Fan pilot."""
...
PILOT = Path(
    ...
    "world_warrior_score_pickup_style_pilot_v1.png"
)
STAGE_CAPTURE = Path(
    "Artifacts/StyleCalibration/"
    "world_warrior_grand_grappler_tetsu_idle_runtime_1280x720.png"
)
STAGE_PROXY = Path(
    "Artifacts/StyleCalibration/"
    "world_warrior_score_pickup_pilot_v1_stage_scale_proxy.png"
)
READABILITY_STRIP = Path(
    "Artifacts/StyleCalibration/"
    "world_warrior_score_pickup_pilot_v1_readability_strip.png"
)
...
def main() -> int:
    if not PILOT.is_file() or not STAGE_CAPTURE.is_file():
        raise FileNotFoundError("pilot or approved Stage 3 capture is missing")

    figure = load_figure()
    stage = Image.open(STAGE_CAPTURE).convert("RGBA")
    stage_figure = resize_to_height(figure, 80)
    stage.alpha_composite(stage_figure, (790, 515 - stage_figure.height))
    stage_draw = ImageDraw.Draw(stage)
    stage_draw.rectangle((774, 394, 1058, 422), fill=(18, 19, 24, 205))
    stage_draw.text(
        (784, 402),
