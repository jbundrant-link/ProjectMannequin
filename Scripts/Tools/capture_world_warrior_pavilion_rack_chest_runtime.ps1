...
$intact = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_pavilion_rack_chest_intact_runtime_$suffix.png"
$drop = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_pavilion_rack_chest_drop_runtime_$suffix.png"
$collection = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_pavilion_rack_chest_collection_runtime_$suffix.png"
$log = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_pavilion_rack_chest_runtime_$suffix.log"
$environment = [ordered]@{
    ...
    PROJECT_MANNEQUIN_STAGE_INDEX = '2'
    ...
    PROJECT_MANNEQUIN_LADDER_PROP_SPRITE_SUFFIX = 'world_warrior_pavilion_rack_chest_style_v1.png'
    PROJECT_MANNEQUIN_LADDER_PICKUP_SPRITE_SUFFIX = 'world_warrior_score_pickup_style_v1.png'
}
$previous = @{}
...
try {
    ...
    $scoreDelta = if ($collectionLines.Count -eq 1) {
        [int]$collectionLines[0].Groups[2].Value `
            - [int]$collectionLines[0].Groups[1].Value
    } else {
        -1
    }
    ...
    try {
        $passed = $exitCode -eq 0 `
            ...
            -and $summary.Value -match 'stage=2' `
        ...
        if (-not $passed) {
            throw "Pavilion Rack Chest runtime capture failed validation at $suffix."
        }
    }
    finally {
        if ($intactImage) {
