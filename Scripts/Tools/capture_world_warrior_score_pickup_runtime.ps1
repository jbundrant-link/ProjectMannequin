...
$intact = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_score_pickup_dummy_runtime_$suffix.png"
$drop = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_score_pickup_drop_runtime_$suffix.png"
$collection = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_score_pickup_collection_runtime_$suffix.png"
$log = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_score_pickup_runtime_$suffix.log"
$environment = [ordered]@{
    ...
    PROJECT_MANNEQUIN_LADDER_PROP_SPRITE_SUFFIX = 'world_warrior_training_dummy_style_v1.png'
...
try {
    ...
    try {
        ...
        if (-not $passed) {
            throw "Judge's Laurel Fan runtime capture failed validation at $suffix."
        }
    }
    finally {
        if ($intactImage) {
