...
$intact = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_meter_pickup_dummy_runtime_$suffix.png"
$drop = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_meter_pickup_drop_runtime_$suffix.png"
$collection = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_meter_pickup_collection_runtime_$suffix.png"
$log = Join-Path $projectRoot "Artifacts\StyleCalibration\world_warrior_meter_pickup_runtime_$suffix.log"
$environment = [ordered]@{
...
try {
    ...
    try {
        ...
        if (-not $passed) {
            throw "Focus Drum runtime capture failed validation at $suffix."
        }
    }
    finally {
        if ($intactImage) {
