#!/usr/bin/env python3
"""Create gameplay-scale review proxies for the World Warrior supply crate pilot."""
...
STAGE_PROXY = Path(
    "Artifacts/StyleCalibration/"
    "world_warrior_supply_crate_pilot_v1_stage_scale_proxy.png"
)
READABILITY_STRIP = Path(
    "Artifacts/StyleCalibration/"
    "world_warrior_supply_crate_pilot_v1_readability_strip.png"
)
...
def main() -> int:
    if not PILOT.is_file() or not STAGE_CAPTURE.is_file():
        raise FileNotFoundError("pilot or approved dummy capture is missing")

    figure = load_figure()
    stage = Image.open(STAGE_CAPTURE).convert("RGBA")
    stage_figure = resize_to_height(figure, 124)
    stage.alpha_composite(stage_figure, (760, 522 - stage_figure.height))
    stage_draw = ImageDraw.Draw(stage)
    stage_draw.rectangle((744, 372, 1028, 400), fill=(18, 19, 24, 205))
    stage_draw.text(
        (754, 380),
