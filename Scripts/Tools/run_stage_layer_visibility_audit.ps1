...
$outputDirectory = Join-Path $projectRoot 'Artifacts\StyleCalibration\LayerVisibility'
$report = Join-Path $projectRoot 'Artifacts\stage_layer_visibility_report.json'

# Every layered stage. Plate stages draw one painting and have no parallax
# layers to suppress.
$stages = @(
...
foreach ($stage in $stages) {
    foreach ($variant in @('reference', 'Far', 'Midground', 'Foreground')) {
        $capturePath = Join-Path $outputDirectory "$($stage.Name)__$variant.png"
        ...
        if ($variant -ne 'reference') { $env:PROJECT_MANNEQUIN_HIDE_LAYER = $variant }
    ...
    $captureArguments += '--stage'
    $captureArguments += $stage.Name
}
...
try {
    & (Join-Path $projectRoot '.venv\Scripts\python.exe') `
        'Scripts/Tools/audit_stage_layer_visibility.py' `
        --directory $outputDirectory @captureArguments --report $report
